...
const Index = () => {
  ...
  useEffect(() => {
    ...
    if (onboardingSeen !== 'true') navigate('/onboarding');
  }, [navigate]);
  ...
  useEffect(() => {
    ...
    check();
    const { data: listener } = supabase.auth.onAuthStateChange((event) => setIsLoggedIn(event === 'SIGNED_IN'));
  ...
  useEffect(() => {
    const fetchRides = async () => {
      ...
      } catch { setPopularRides(getMockRides().slice(0, 4)); }
    };
    fetchRides();
  }, []);

  const t = (fr: string, ar: string, en: string) => language === 'fr' ? fr : language === 'ar' ? ar : en;
    ...
    <div className="pb-20 pt-16">
      <motion.div variants={container} initial="hidden" animate="show" className="px-4 max-w-lg mx-auto space-y-6">
        ...
        <motion.div variants={item} className="pt-4">
          <p className="text-muted-foreground text-sm">{t('Bonjour 👋', 'مرحبا 👋', 'Hello 👋')}</p>
          <h1 className="text-2xl font-bold text-foreground mt-1">{t('Où allez-vous ?', 'إلى أين تذهب؟', 'Where are you going?')}</h1>
        </motion.div>

        {/* Search */}
        <motion.div variants={item}>
          <button onClick={() => navigate('/rides')} className="w-full flex items-center gap-3 bg-muted/60 hover:bg-muted rounded-2xl px-4 py-3.5 transition-colors">
            <Search className="w-5 h-5 text-muted-foreground" />
            <span className="text-muted-foreground text-sm">{t('Rechercher une destination...', '...ابحث عن وجهة', 'Search a destination...')}</span>
          </button>
        </motion.div>

        {/* Quick Actions */}
        <motion.div variants={item}>
          <h2 className="text-base font-semibold text-foreground mb-3">{t('Actions rapides', 'إجراءات سريعة', 'Quick Actions')}</h2>
          <div className="grid grid-cols-2 gap-3">
            {quickActions
              .filter(a => !(a.hideWhenLoggedIn && isLoggedIn))
              .map((action) => {
                const content = (
                  <motion.div
                    whileTap={{ scale: 0.96 }}
                    className="flex flex-col items-center gap-2.5 p-4 rounded-2xl bg-card border border-border shadow-sm hover:shadow-md transition-all active:scale-95 cursor-pointer"
                  >
                ...
                if (action.href) {
                  return <a key={action.label} href={action.href} target="_blank" rel="noopener noreferrer">{content}</a>;
                }
                return <div key={action.label} onClick={() => navigate(action.path!)}>{content}</div>;
              })}
          </div>
        </motion.div>
        ...
        {/* Why Wasslink */}
        <motion.div variants={item}>
          <h2 className="text-base font-semibold text-foreground mb-3">{t('Pourquoi Wasslink ?', 'لماذا Wasslink؟', 'Why Wasslink?')}</h2>
          <div className="flex gap-3">
            {[
              { icon: Shield, label: t('Sécurisé', 'آمن', 'Safe'), desc: t('Voyages vérifiés', 'رحلات موثقة', 'Verified rides') },
              { icon: Clock, label: t('Rapide', 'سريع', 'Fast'), desc: t('Réservation rapide', 'حجز سريع', 'Quick booking') },
              { icon: Users, label: t('Communauté', 'مجتمع', 'Community'), desc: t('Conducteurs fiables', 'سائقون موثوقون', 'Trusted drivers') },
            ].map((f) => (
        ...
        <motion.div variants={item}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-semibold text-foreground">{t('Trajets populaires', 'الرحلات الشائعة', 'Popular Rides')}</h2>
            <button onClick={() => navigate('/rides')} className="text-xs font-medium text-primary flex items-center gap-1">
              {t('Voir tout', 'عرض الكل', 'See All')} <ArrowRight className="w-3 h-3" />
            </button>
          </div>
            ...
            {popularRides.map((ride) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#C7FFD6] to-[#94C5FF] flex items-center justify-center text-sm font-bold text-foreground shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      ...
                      <span className="text-[10px] text-muted-foreground bg-muted rounded-full px-2 py-0.5">{ride.seats} {t('places', 'مقاعد', 'seats')}</span>
                      <div className="flex items-center gap-0.5 text-[10px] text-yellow-500">
                        <Star className="w-3 h-3 fill-current" /> {ride.rating}
                      </div>
                    </div>
                  </div>
                </div>
